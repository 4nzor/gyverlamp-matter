...
#if defined(ESP32)
#include "esp32_fix.h"  // Определения макросов для ESP32 (SEMAPHORE_TAKE, SEMAPHORE_GIVE)

// ПАТЧ ДЛЯ ESP32-C3: В новой версии FastLED (3.10.4+) используется RMT драйвер
// ESP32-C3 имеет архитектуру RISC-V, и FastLED автоматически определяет RMT поддержку
// RMT драйвер работает через FASTLED_ESP32_HAS_RMT (определяется автоматически)
#if defined(CONFIG_IDF_TARGET_ESP32C3) || defined(ARDUINO_ESP32C3_DEV)
  // Отключаем I2S для ESP32-C3 (I2S не поддерживается на RISC-V архитектуре)
  #undef FASTLED_ESP32_I2S
  // RMT драйвер будет выбран автоматически через FASTLED_ESP32_HAS_RMT
  // Если RMT недоступен, FastLED автоматически выберет альтернативный метод
#endif

// ПАТЧ ДЛЯ ESP32-C3: FastLED требует esp_intr_alloc.h вместо esp_intr.h
// Патч уже применен автоматически к файлам FastLED:
// - clockless_rmt_esp32.h: esp_intr.h → esp_intr_alloc.h
// - clockless_i2s_esp32.h: esp_intr.h → esp_intr_alloc.h
// Если ошибка сохраняется, проверьте, что патч применен к библиотеке FastLED
#endif

#include "pgmspace.h"
#include "Constants.h"

// Forward declaration функции XY() для FastLED 3.10.4+
// Должна быть объявлена ДО включения FastLED.h, чтобы FastLED мог найти её
...
// Определен здесь, чтобы быть доступным во всех функциях, включая setup()
#if defined(ESP8266)
...
uint16_t XY(uint8_t x, uint8_t y);  // Изменено на uint8_t для совместимости с FastLED 3.10.4+
...
{
  Serial.begin(115200);
  delay(500); // Даем время на инициализацию Serial
  Serial.println();
  Serial.println(F("=== GyverLamp Starting ==="));
  #if defined(ESP8266)
    Serial.print(F("Platform: ESP8266, Chip ID: "));
    Serial.println(ESP.getChipId());
  #elif defined(ESP32)
    Serial.print(F("Platform: ESP32"));
    #if defined(CONFIG_IDF_TARGET_ESP32C3)
      Serial.println(F(" (ESP32-C3)"));
      // Для ESP32-C3 можно снизить частоту CPU для экономии энергии и стабильности
      // По умолчанию ESP32-C3 работает на 160MHz, можно снизить до 80MHz если есть проблемы
      // setCpuFrequencyMhz(80); // Раскомментируйте, если есть проблемы с питанием
    #else
      Serial.println();
    #endif
  ...
  // Задержка для стабилизации питания после включения
  delay(200);
  WDT_FEED();
  ...
  // ЛЕНТА/МАТРИЦА
  // Задержка перед инициализацией FastLED для стабилизации питания
  delay(100);
  WDT_FEED();
  ...
  Serial.println(NUM_LEDS);
    ...
    Serial.println(F("ESP32-C3 detected"));
    Serial.println(F("Using RMT driver for ESP32-C3 (auto-detected by FastLED)"));
  #endif
  Serial.flush();
  
  // Для всех ESP32 (включая ESP32-C3) используем стандартный метод
  // Патч для esp_intr.h применен выше в коде
  ...
  Serial.println(F("FastLED.addLeds() completed"));
  Serial.flush();
  WDT_FEED();
  delay(50); // Задержка для стабилизации после инициализации FastLED
  
  FastLED.setBrightness(BRIGHTNESS);
  Serial.println(F("FastLED.setBrightness() completed"));
  Serial.flush();
  
  if (CURRENT_LIMIT > 0)
  {
    FastLED.setMaxPowerInVoltsAndMilliamps(5, CURRENT_LIMIT);
    Serial.println(F("FastLED.setMaxPowerInVoltsAndMilliamps() completed"));
    Serial.flush();
  }
  WDT_FEED();
  delay(50); // Задержка перед первым выводом на ленту
  
  FastLED.clear();
  Serial.println(F("FastLED.clear() completed"));
  Serial.flush();
  
  FastLED.show();
  Serial.println(F("FastLED.show() completed - FastLED initialized"));
  Serial.flush();
  WDT_FEED();
  delay(100); // Задержка после первого вывода на ленту для стабилизации питания
  ...
  Serial.flush();
  // Инициализация espMode по умолчанию, если EEPROM не инициализирован
  // espMode объявлен в Constants.h и инициализирован значением ESP_MODE
  Serial.print(F("ESP_MODE (default): "));
  ...
  Serial.print(F("Рабочий режим лампы: ESP_MODE = "));
  Serial.println(espMode);
  Serial.flush();
  ...
  if (espMode == 0U)                                        // режим WiFi точки доступа
  {
    ...
    Serial.flush();
    WDT_FEED();
    delay(300); // Задержка перед запуском WiFi AP для стабилизации питания
    ...
    WDT_FEED();
    delay(100); // Задержка после запуска WiFi AP
  ...
  {
    Serial.println(F("Starting WiFi client mode..."));
    Serial.flush();
    LOG.println(F("Старт в режиме WiFi клиента (подключение к роутеру)"));

    if (strlen(STA_SSID) > 0)                               // SSID и пароль жёстко заданы в Constants.h
    {
      ...
      LOG.printf_P(PSTR("Подключение к заданной WiFi сети: %s\n"), STA_SSID);
      Serial.print(F("Fixed WiFi: "));
      ...
      while (WiFi.status() != WL_CONNECTED && (millis() - wifiStart) < (ESP_CONN_TIMEOUT * 1000UL))
      {
        delay(500);
        WDT_FEED();
        Serial.print('.');
      ...
      {
        LOG.println(F("Не удалось подключиться к заданной WiFi сети\nПроверьте STA_SSID и STA_PASS в Constants.h\nРестарт...\n"));
    ...
    else                                                    // STA_SSID пустой — старый WiFiManager
    {
    if (WiFi.SSID().length())
    {
  ...
  ESP.wdtFeed();                                            // пнуть собаку
    ...
    Serial.flush();
    WDT_FEED();
    delay(200); // Задержка перед подключением к WiFi для стабилизации питания после FastLED
    ...
    Serial.println(F("wifiManager.autoConnect() returned"));
    Serial.flush();
    WDT_FEED();
    delay(200); // Задержка после подключения к WiFi для стабилизации питания
  ...
  Serial.flush();
  WDT_FEED();
  delay(100); // Задержка перед финализацией для стабилизации питания
  
  memset(matrixValue, 0, sizeof(matrixValue)); //это массив для эффекта Огонь. странно, что его нужно залить нулями
  randomSeed(micros());
  WDT_FEED();
  
  Serial.println(F("Calling changePower()..."));
  Serial.flush();
  changePower();
  loadingFlag = true;
  WDT_FEED();
  delay(100); // Задержка после changePower для стабилизации питания
...
void loop()
{
  ...
  // Периодический watchdog feed в loop (каждые 50ms для ESP32-C3)
  if (millis() - lastWdtFeed > 50) {
    WDT_FEED();
    lastWdtFeed = millis();
  }
...
if (Painting == 0) {
  ...
  WDT_FEED(); // Feed watchdog в конце loop
