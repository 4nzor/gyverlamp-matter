...
// ВАЖНО: Сигнатура должна быть uint16_t XY(uint8_t x, uint8_t y) для совместимости с FastLED 3.10.4+
...
// Переопределение для FastLED 3.10.4+ (namespace fl::)
// FastLED использует слабый символ в namespace fl::, который нужно переопределить
namespace fl {
  uint16_t XY(uint8_t x, uint8_t y) {
    // Вызываем глобальную функцию XY()
    return ::XY(x, y);
  }
}
...
{
    EepromManager::RestoreDefaultModesSettings(modes);
    EepromManager::RestoreDefaultGlobalSettings();
}
