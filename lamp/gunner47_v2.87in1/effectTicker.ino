...
void effectsTick()
{
  static uint8_t lastMode = 255; // Инициализируем значением, которое точно не совпадет с реальным режимом
  
  // Сбрасываем таймер при смене эффекта
  ...
  if (!dawnFlag)
  {
    ...
    // Используем безопасное вычисление с проверкой на переполнение millis()
    uint32_t currentTime = millis();
    uint32_t elapsed = (currentTime >= effTimer) ? (currentTime - effTimer) : (0xFFFFFFFFU - effTimer + currentTime + 1U);
    // Задержки для эффектов с управлением скоростью через ползунок
    uint32_t delayTime;
    if (currentMode >= EFF_MATRIX) {
      // Эффекты начиная с EFF_MATRIX используют ползунок скорости напрямую
      delayTime = 256U - modes[currentMode].Speed;
    } else if (currentMode <= EFF_OCEAN) {
      // Первые 11 эффектов (шумы) - фиксированная задержка 50ms
      delayTime = 50U;
    } else {
      // Эффекты между EFF_OCEAN и EFF_MATRIX (12U-37U) - управление скоростью с ограничением максимума
      // Спирали, Призмата, Дымовые шашки, Тихий океан, Стая, Шары, Лавовая лампа, 
      // Парящий огонь, Верховой огонь, Дым, Пикассо, Волны, Разноцветный дождь, Прыгуны, Пейнтбол
      // Используем ползунок скорости, но ограничиваем максимальную задержку для предотвращения слишком медленной работы
      uint32_t calculatedDelay = 256U - modes[currentMode].Speed;
      delayTime = (calculatedDelay > 100U) ? 100U : calculatedDelay; // Максимум 100ms - эффекты не будут работать слишком медленно
    }
    
    if (ONflag && elapsed >= delayTime)
    {
      effTimer = currentTime;
      WDT_FEED(); // Watchdog feed перед выполнением эффекта
      switch (currentMode)
      {
        ...
        case EFF_FIREWORKS:           fireworksRoutine();                 break;  // (86U) Фeйepвepк
        case EFF_PLASMA_WAVES:         plasmaRoutine();                    break;  // (87U) Плaзмa вoлн
        case EFF_VORTEX:               vortexRoutine();                    break;  // (88U) Bиxpи
