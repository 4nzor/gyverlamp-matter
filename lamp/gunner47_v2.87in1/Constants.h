...
//#define USE_DEFAULT_SETTINGS_RESET                        // если раскомментировать: LIST3 сбросит настройки эффектов на default (мешает веб-пульту / приложениям, которые только читают список)
...
// #define GENERAL_DEBUG        (true)                             // если строка не закомментирована, будут выводиться отладочные сообщения

#define LED_PIN               (4U)                          // пин ленты                (D4) - встроенный светодиод платы тоже на D4, поэтому лучше паять на D3 = #define LED_PIN (0U)
// #define BTN_PIN               (3U)                          // пин кнопки               (D2)
...
// ВАЖНО ДЛЯ ESP32-C3: Если лампа зависает при запуске или во время работы эффектов, возможно проблема с питанием:
// 
// ДИАГНОСТИКА ПРОБЛЕМ С ПИТАНИЕМ:
// 1. ТЕСТ: Уменьшите яркость в приложении до 10-20%. Если эффекты стали работать плавно и быстро - 
//    проблема точно в питании (недостаточный ток или просадка напряжения).
//
// 2. ТРЕБОВАНИЯ К БЛОКУ ПИТАНИЯ:
//    - Для 256 LED (16x16) рекомендуется минимум 3-5A при 5V
//    - Блок питания должен быть качественным, с запасом по току
//    - Напряжение должно быть стабильным (5V ± 0.2V)
//
// 3. ПРОВОДА ПИТАНИЯ:
//    - Сечение проводов должно быть не менее 0.75мм² (лучше 1.0мм²)
//    - Длина проводов от блока питания до ленты должна быть минимальной
//    - Используйте отдельные провода для питания ленты и ESP (не через один провод)
//
// 4. КОНДЕНСАТОР:
//    - Установите конденсатор 1000-2200 мкФ 6.3V+ рядом с началом ленты
//    - Это сглаживает пики потребления при быстрой смене цветов
//
// 5. НАСТРОЙКА СКОРОСТИ В ПРИЛОЖЕНИИ:
//    - Если эффекты работают медленно или рывками, попробуйте:
//      * Установить ползунок "Скорость" на 30-50% (не на максимум!)
//      * Увеличить ползунок "Масштаб" (Шаг) для более плавной анимации
//    - При уменьшенной задержке между кадрами большие значения Speed могут вызывать рывки
//    - Оптимальные значения: Speed = 100-150, Scale = 40-60
//
// 6. ЕСЛИ ПРОБЛЕМА СОХРАНЯЕТСЯ:
//    - Попробуйте увеличить CURRENT_LIMIT в Constants.h до 3000U или 5000U
//    - Или снизите частоту CPU: Tools -> CPU Frequency -> 80 MHz в Arduino IDE
//    - Или программно в gunner47_v2.87in1.ino раскомментировать: setCpuFrequencyMhz(80);
//    - Проверьте, нет ли перегрева блока питания (он должен быть прохладным)


// --- ESP (WiFi клиент) ---------------
                                                            // Если STA_SSID не пустой — лампа подключается ТОЛЬКО к этой сети (без портала WiFiManager).
                                                            // Если STA_SSID оставить "" — как раньше: точка доступа AP_NAME и ввод пароля с телефона.
                                                            // Секреты: wifi_secrets.h (из secrets.env → ./scripts/apply_secrets.sh) или скопируй wifi_secrets.h.example.
...
#endif
#ifndef STA_SSID
#define STA_SSID              ""                            // имя WiFi-сети роутера
#endif
#ifndef STA_PASS
#define STA_PASS              ""                            // пароль этой сети (обязательно заполнить при STA_SSID)
#endif
...
#define ESP_CONN_TIMEOUT      (30U)                         // время в секундах, которое ESP будет пытаться подключиться к WiFi сети, после его истечения автоматически развернёт WiFi точку доступа
...
#define AP_PASS               ("12345678")                  // пароль WiFi точки доступа (смени после первой настройки)
...
// --- МАТРИЦА -------------------------
// ВАЖНО: Если эффекты работают медленно или лампа зависает, проверьте питание!
// Диагностика проблем с питанием:
// 1. Уменьшите яркость в приложении до 10-20%. Если эффекты стали работать плавно - проблема в питании.
// 2. Проверьте блок питания: для 256 LED (16x16) рекомендуется минимум 3-5A при 5V.
// 3. Проверьте провода питания: сечение должно быть не менее 0.75мм², длина минимальная.
// 4. Установите конденсатор 1000-2200 мкФ рядом с лентой для сглаживания пиков потребления.
// 5. Если проблема сохраняется, попробуйте увеличить CURRENT_LIMIT или установить 0 (отключить ограничение).
//    Но будьте осторожны - это может привести к перегреву блока питания!
#define CURRENT_LIMIT         (1000U)                       // лимит по току в миллиамперах, автоматически управляет яркостью (пожалей свой блок питания!) 0 - выключить лимит
                                                            // Для 256 LED рекомендуется 3000-5000 мА. Если эффекты медленные - попробуйте увеличить до 3000U или 5000U
...
// --- ЭФФЕКТЫ -------------------------
#define RUNNING_TEXT_DEFAULT  ("v.0.0.5")                    // текст, который будет выводиться в эффекте Бегущая строка по умолчанию. его можно менять в приложении
...
#define EFF_FIREWORKS           (86U)    // Фeйepвepк
#define EFF_PLASMA_WAVES        (87U)    // Плaзмa вoлн
#define EFF_VORTEX              (88U)    // Bиxpи
#define EFF_CRYSTALS            (89U)    // Кpиcтaллы
#define EFF_LIGHTNING           (90U)    // Moлнии
#define EFF_GRAVITY             (91U)    // Гpaвитaция
#define EFF_FACTORIAL           (92U)    // Фaктopиaл
...
String("86. Фeйepвepк,50,255,1,100,0;") +
String("87. Плaзмa вoлн,50,255,1,100,0;") +
String("88. Bиxpи,50,255,1,100,0;") +
String("89. Кpиcтaллы,50,255,1,100,0;") +
String("90. Moлнии,50,255,1,100,0;") +
String("91. Гpaвитaция,50,255,1,100,0;") +
String("92. Фaктopиaл,40,255,1,100,0;") +
...
#endif
#if defined(ESP32)
// Для ESP32 FastLED использует другой порядок пинов по умолчанию
  #if defined(CONFIG_IDF_TARGET_ESP32C3)
  // ESP32-C3 использует RMT для управления WS2812B
  // FastLED автоматически определяет ESP32-C3 и использует правильный драйвер
  #endif
#endif
