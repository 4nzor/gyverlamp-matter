...
void parseUDP()
{
  ...
  if (packetSize)
  {
    ...
    // Для команды LIST нужно начать пакет ДО вызова processInputBuffer(), 
    // чтобы команда могла отправлять данные в том же пакете
    bool isListCommand = !strncmp_P(inputBuffer, PSTR("LIST"), 4);
    ...
    // Для команды LIST данные уже отправлены в processInputBuffer(), отправляем только если reply не пуст
    if (strlen(reply) > 0) {
    #if defined(ESP8266)
        Udp.write(reply);
    #endif
...
{
  WDT_FEED(); // Feed watchdog в начале обработки команды
...
if (!strncmp_P(inputBuffer, PSTR("GET"), 3))
{
  ...
  {
    if (strlen(inputBuffer) > 7U){
      len = strlen(inputBuffer) - 4;
      if (len > MAX_UDP_BUFFER_SIZE - 1) len = MAX_UDP_BUFFER_SIZE - 1;
      memcpy(buff, &inputBuffer[4], len);
      buff[len] = '\0';
  ...
  sendCurrent(inputBuffer);
    ...
    else if (!strncmp_P(inputBuffer, PSTR("EFF"), 3))
    {
      ...
      {
        currentMode = newMode;
        loadingFlag = true;                                  // устанавливаем флаг загрузки для инициализации эффекта
        FastLED.setBrightness(modes[currentMode].Brightness);
        updateSets();
        sendCurrent(inputBuffer);
        #ifdef USE_BLYNK_PLUS
...
{
  WDT_FEED(); // Feed watchdog в начале sendCurrent
