...
static bool startButtonHolding = true;                     // флаг: кнопка удерживается для изменения яркости/скорости/масштаба лампы кнопкой
...
void buttonTick()
{
  ...
  else if (clickCount == 5U)                                     // вывод IP на лампу
  {
    if (espMode == 1U)
    {
      ...
      while(!fillString(WiFi.localIP().toString().c_str(), CRGB::White, false)) { delay(1);
       #if defined(ESP8266)
